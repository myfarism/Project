    ...
    def akhiri_sewa(kode_laptop):
        try:
            ...
            if connection.is_connected():
                ...
                query_mulai = "SELECT jam_mulai FROM sewa_laptop WHERE kode_laptop = %s AND jam_akhir IS NULL"
                cursor.execute(query_mulai, (kode_laptop,))
                jam_mulai = cursor.fetchone()[0]

                # Perbarui jam akhir sewa
                update_query = "UPDATE sewa_laptop SET jam_akhir = NOW() WHERE kode_laptop = %s AND jam_akhir IS NULL"
                ...
                query_akhir = "SELECT jam_akhir FROM sewa_laptop WHERE kode_laptop = %s ORDER BY created_at DESC LIMIT 1"
                cursor.execute(query_akhir, (kode_laptop,))
